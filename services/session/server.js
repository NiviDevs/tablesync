...
async function start() { await database.query('SELECT 1'); await redis.connect(); const connection = await amqp.connect(process.env.RABBITMQ_URL ?? 'amqp://guest:guest@localhost:5672'); const channel = await connection.createChannel(); await channel.assertExchange('tablesync.events', 'topic', { durable: true }); await channel.assertQueue('tablesync.audit', { durable: true }); await channel.bindQueue('tablesync.audit', 'tablesync.events', '#'); broker = channel; const app = buildApp(); await app.listen({ port: Number(process.env.PORT ?? 8081), host: '0.0.0.0' }); }
